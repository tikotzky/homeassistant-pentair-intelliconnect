...
│   ├── __init__.py          # Exports PentairPoolDataUpdateCoordinator
...
│   ├── __init__.py          # Exports PentairPoolEntity
...
- `base.py` - Main coordinator class (`PentairPoolDataUpdateCoordinator`)
...
**Key class:** `PentairPoolDataUpdateCoordinator` (exported from `coordinator/__init__.py`)
...
**Key class:** `PentairPoolApiClient`
...
- `PentairPoolConfigFlowHandler` (main flow)
- `PentairPoolOptionsFlow` (options)
...
**Key class:** `PentairPoolEntity` (in `entity/base.py`)
...
2. `PentairPoolEntity` for common functionality
...
1. Create directory: `custom_components/pentair_pool/<platform>/`
2. Implement `__init__.py` with `async_setup_entry()`
3. Create entity classes inheriting from platform base + `PentairPoolEntity`
