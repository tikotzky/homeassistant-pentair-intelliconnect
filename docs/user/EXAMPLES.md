...
with the Pentair IntelliConnect custom integration.
    ...
    condition:
      - condition: time
        weekday:
          - mon
    action:
      - action: pentair_pool.example_service
        target:
...
```yaml
blueprint:
  name: Pentair IntelliConnect — Threshold Alert
...
- [GitHub Issues](https://github.com/tikotzky/homeassistant-pentair-pool/issues) - Report problems
