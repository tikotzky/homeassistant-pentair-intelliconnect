...
- Files: `snake_case.py`
- Classes: `PascalCase` prefixed with `PentairPool`
...
- Inherit from both platform entity and `PentairPoolEntity` (order matters)
...
from custom_components.pentair_pool.sensor import async_setup_entry
...
**Config entry data:** `entry_data: PentairPoolData = hass.data[DOMAIN][entry.entry_id]`
